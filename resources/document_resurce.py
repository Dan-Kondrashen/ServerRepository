...
from sqlalchemy import and_, or_, outerjoin, desc, asc, func, case, not_, cast, Integer
...
from reqparsers.document_reqparse import parser, fulldocumentparser, parserFilter, parserFilterFull
...
class DocumentFiltrationResource(Resource):
    def post(self, user_id, type, mod):
        session = db_sessions.create_session()
        try:
            date_format = "%Y-%m-%d %H:%M:%S"
            args = parserFilterFull.parse_args()
            knowledgesArg = args["knowIdList"]
            resultKnow =[]
            ...
            if not resultKnow:
                query = session.query(Document).filter(not_(Document.id.in_(subquery))).filter(Document.type == type)
                if not args["salaryS"] == None:
                    query = query.filter(or_(Document.salaryS == None,  cast(Document.salaryS, Integer) <= int(args["salaryS"])))
                if not args["salaryF"] == None:
                    query = query.filter(or_(Document.salaryF == None,  cast(Document.salaryF, Integer) >= int(args["salaryF"])))
                if not args["dateStart"] == None:
                    query = query.filter(Document.date >= datetime.datetime.strptime(args["dateStart"], date_format))
                if not args["dateEnd"] == None:
                    query = query.filter(Document.date < datetime.datetime.strptime(args["dateEnd"], date_format))
                if args["orderType"] == "date":
                    if mod == "desc":
                        query = query.order_by(desc(Document.date))
                    else:
                        query = query.order_by(asc(Document.date))
                elif args["orderType"] == "number":
                    if mod == "desc":
                        query = query.join(DocumentViews).order_by(desc(func.sum(DocumentViews.numUsages))).filter(DocumentViews.type == "view", DocumentViews.numUsages != 0).group_by(Document.id)
                    else:
                        query = query.join(DocumentViews).order_by(asc(func.sum(DocumentViews.numUsages))).filter(DocumentViews.type == "view", DocumentViews.numUsages != 0).group_by(Document.id)
                elif args["orderType"] == "wage":
                    if mod == "desc":
                        query = query.order_by(desc(Document.salaryF), desc(Document.salaryS))
                    else:
                        query = query.order_by(asc(Document.salaryF), asc(Document.salaryS))
                elif args["orderType"] == "level":
                    if mod == "desc":
                        query = query.join(UserCurLevel, UserCurLevel.userId == Document.userId).join(Level, UserCurLevel.levelId == Level.id).order_by(desc(Level.number))
                    else:
                        query = query.join(UserCurLevel, UserCurLevel.userId == Document.userId).join(Level, UserCurLevel.levelId == Level.id).order_by(asc(Level.number))
                if not args["numItems"] == None:
                    print("ограничение выборки " + args["numItems"])
                    query = query.limit(int(args["numItems"]))
            else:    
                query = session.query(Document).join(Document.knowledge).filter(not_(Document.id.in_(subquery))).filter(Document.type == type, Knowledge.id.in_(resultKnow))
                if not args["salaryS"] == None:
                    query = query.filter(or_(Document.salaryS == None, Document.salaryS <= args["salaryS"]))
                if not args["salaryF"] == None:
                    query = query.filter(or_(Document.salaryF == None, Document.salaryF >= args["salaryF"]))
                if not args["dateStart"] == None:
                    query = query.filter(Document.date >= datetime.datetime.strptime(args["dateStart"], date_format))
                if not args["dateEnd"] == None:
                    query = query.filter(Document.date < datetime.datetime.strptime(args["dateEnd"], date_format))
